...
export class GitHubClient {
  ...
  async getSource(path: string, ref?: string): Promise<GitHubSource> { const clean = path.replace(/^\/+/, ''); if (!clean || clean.split('/').some(segment => segment === '..')) throw new IntegrationError('Invalid GitHub source path', 'inconclusive'); const encoded = clean.split('/').map(encodeURIComponent).join('/'); const q = ref ? `?ref=${encodeURIComponent(ref)}` : ''; const r = await requestJson<{ content: string; encoding: string; sha: string; path: string }>(this.path(`/contents/${encoded}${q}`), { headers: this.headers() }, this.options.fetchFn); if (typeof r.content !== 'string' || typeof r.sha !== 'string') throw new IntegrationError('GitHub source response was malformed', 'inconclusive'); return { path: r.path, ref, sha: r.sha, content: r.encoding === 'base64' ? Buffer.from(r.content.replace(/\n/g, ''), 'base64').toString('utf8') : r.content }; }
  ...
  async listChecks(ref: string): Promise<GitHubCheck[]> { const r = await requestJson<{ check_runs: GitHubCheck[] }>(this.path(`/commits/${encodeURIComponent(ref)}/check-runs`), { headers: this.headers() }, this.options.fetchFn); if (!Array.isArray(r.check_runs)) throw new IntegrationError('GitHub checks response was malformed', 'inconclusive'); return r.check_runs; }
